 ...
 *
 * MUSA port of reduce/sum benchmark.
 ******************************************************************************/
...
void run_benchmark(int64_t elements, bool output_json)
{
  ...
  using output_it_t = T *;
  using offset_t    = int;  // Use int for offset
  using output_t    = T;
  using init_t      = T;
...
int main(int argc, char **argv)
{
  ...
  if (!output_json) {
    std::cout << "=== Benchmark: cub::DeviceReduce::Sum ===" << std::endl;
  }
